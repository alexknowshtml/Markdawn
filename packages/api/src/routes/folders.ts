...
const ensureNoFolderCycle = async (workspaceId: string, folderId: string, targetParentId: string | null) => {
  if (!targetParentId) {
    return;
  }
  ...
  (result.rows as { id: string; parent_id: string | null }[]).forEach((row) => {
...
foldersRoute.patch(":id", async (c) => {
  ...
  const nextName = typeof name === "string" ? (name.trim().length > 0 ? name.trim() : "New Folder") : folder.name;
  const nextIcon = typeof icon === "string" ? (icon.trim().length > 0 ? icon.trim() : null) : icon === null ? null : folder.icon;
  const nextPosition = typeof position === "string"
    ? (position.trim().length > 0 ? position.trim() : folder.position)
...
foldersRoute.post(":id/copy", async (c) => {
  ...
  try {
    ...
    throw new HTTPException(500, { message: err instanceof Error ? err.message : "Failed to copy folder" });
  }
});
...
async function copyFolderRecursive(
  ...
  for (const pageRow of pagesResult.rows) {
    ...
    await pool.query(
      ...
      [workspaceId, newFolder.id, `Copy of ${pr.title}`, pr.icon, pr.cover_type, pr.cover_value, pr.position, pr.ydoc, userId]
    );
  }
  ...
  for (const subfolderRow of subfoldersResult.rows) {
    await copyFolderRecursive((subfolderRow as { id: string }).id, newFolder.id, workspaceId, userId);
  }
...
foldersRoute.delete(":id", async (c) => {
  ...
  const hasChildren = (directChildFolders.rowCount ?? 0) > 0 || (directChildPages.rowCount ?? 0) > 0;
