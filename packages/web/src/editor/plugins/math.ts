...
import { $nodeSchema, $inputRule, $remark, $command, $view } from "@milkdown/kit/utils";
...
import { nodeRule, findNodeInSelection } from "@milkdown/kit/prose";
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- remark-math types are not portable
...
export const toggleLatexCommand = $command(
  "ToggleLatex",
  (ctx: Ctx) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Command types are complex
    return () => (state: any, dispatch?: any) => {
      const { hasNode, pos: latexPos, target: latexNode } = findNodeInSelection(
        state,
        mathInlineSchema.type(ctx),
      );
      ...
      if (!hasNode) {
        ...
        if (dispatch) {
          dispatch(
            newTr.setSelection(
              NodeSelection.create(newTr.doc, selection.from),
            ),
          );
        }
        return true;
      ...
      if (dispatch) {
        dispatch(
          newTr.setSelection(
            TextSelection.create(newTr.doc, latexPos, latexPos + content.length),
          ),
        );
      }
      return true;
  ...
  dom.addEventListener("click", (e) => {
    ...
    mathEditor.open(view, dom, {
      ...
      onConfirm: (newValue) => {
        ...
        const newCodeBlock = view.state.schema.nodes.code_block!.create(attrs, textNode);
