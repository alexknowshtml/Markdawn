...
async function fetchPagesByTag(workspaceId: string, tagId: string): Promise<{ id: string; title: string; icon: string | null; parentId: string | null }[]> {
  const res = await fetch(`${API_BASE}/tags/pages?workspaceId=${encodeURIComponent(workspaceId)}&tagId=${encodeURIComponent(tagId)}`);
  if (!res.ok) {
...
export function usePagesByTag(workspaceId?: string, tagId?: string) {
  return useQuery({
    ...
    queryFn: () => fetchPagesByTag(workspaceId!, tagId!),
